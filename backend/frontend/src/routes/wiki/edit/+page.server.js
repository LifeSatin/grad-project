...
export async function load({ url }) {
    const res = await fetch(`https://3.27.115.22.nip.io:8443/wiki?pageName=${url.searchParams.get('pageName')}`);
...
export const actions = {
    default: async ({request, url}) => {
        ...
        const res = await fetch(`https://3.27.115.22.nip.io:8443/wiki`, {
            method: "PATCH",
            body: formData,
        });
        const item = await res.json();
